...
export default function NetworkConfig({ open, onClose }: NetworkConfigProps) {
  const {
    config,
    interfaces,
    localIP,
              ...
              <div className="p-3 rounded-xl bg-[rgba(0,40,80,0.2)] border border-[rgba(0,180,255,0.1)]">
                ...
                <div className="space-y-2">
                  {[
                    ...
                    { label: "连接类型", value: (navigator as Navigator & { connection?: { effectiveType?: string; downlink?: number; rtt?: number } }).connection?.effectiveType || "4g" },
                    { label: "下行带宽", value: `${(navigator as Navigator & { connection?: { effectiveType?: string; downlink?: number; rtt?: number } }).connection?.downlink || "~10"} Mbps` },
                    { label: "RTT", value: `${(navigator as Navigator & { connection?: { effectiveType?: string; downlink?: number; rtt?: number } }).connection?.rtt || "~50"} ms` },
                  ].map((item) => (
