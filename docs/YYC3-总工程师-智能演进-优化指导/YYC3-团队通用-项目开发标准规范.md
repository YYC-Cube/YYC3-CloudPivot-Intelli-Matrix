...
所有工作必须遵循 **"分析建议 → 实施推进 → 语法闭环 → 测试用例同步 → 阶段结果闭环 → 上下文衔接 → 进行下阶段工作"** 的完整闭环流程，确保每个阶段都有明确的输入、输出和验证标准。
...
│           ▼                                                                 │
│    ┌──────────────┐                                                         │
│    │ 测试用例同步 │ ─── 单元测试、集成测试、E2E 测试                        │
│    └──────┬───────┘                                                         │
│           │                                                                 │
│           ▼                                                                 │
│    ┌──────────────┐                                                         │
│    │ 阶段结果闭环 │ ─── 验收标准、文档更新、版本发布                        │
│    └──────┬───────┘                                                         │
│           │                                                                 │
│           ▼                                                                 │
│    ┌──────────────┐                                                         │
│    │ 上下文衔接   │ ─── 状态传递、知识沉淀、经验总结                        │
│    └──────┬───────┘                                                         │
│           │                                                                 │
│           └────────────────▶ 进行下阶段工作 (回到分析建议)                  │
│                                                                             │
...
| 要求项 | 说明 | 输出物 |
|--------|------|--------|
| 需求理解 | 深度理解业务需求和技术需求 | 需求分析文档 |
| 可行性评估 | 技术、资源、时间可行性分析 | 可行性报告 |
| 方案设计 | 提供多方案对比和推荐 | 技术方案文档 |
| 风险识别 | 识别潜在风险和应对策略 | 风险清单 |
...
| 要求项 | 说明 | 输出物 |
|--------|------|--------|
| 任务拆解 | 将需求拆解为可执行的开发任务 | 任务清单 |
| 编码实现 | 按照代码规范进行开发 | 源代码 |
| 代码审查 | 提交 PR 并进行 Code Review | 审查记录 |
| 功能集成 | 集成到主分支并验证 | 集成报告 |
...
| 要求项 | 说明 | 输出物 |
|--------|------|--------|
| 类型检查 | TypeScript 严格模式无错误 | 类型检查报告 |
| 代码规范 | ESLint / Prettier 无警告 | 规范检查报告 |
| 静态分析 | 无安全漏洞和代码异味 | 静态分析报告 |
| 依赖检查 | 无已知漏洞的依赖 | 依赖审计报告 |
...
| 要求项 | 说明 | 输出物 |
|--------|------|--------|
| 单元测试 | 核心逻辑覆盖率 ≥ 80% | 测试报告 |
| 集成测试 | 模块间交互测试 | 集成测试报告 |
| E2E 测试 | 关键用户流程测试 | E2E 测试报告 |
| 性能测试 | 性能基准测试 | 性能报告 |
...
| 要求项 | 说明 | 输出物 |
|--------|------|--------|
| 功能验收 | 对照验收标准逐项检查 | 验收报告 |
| 文档更新 | 更新技术文档和用户文档 | 更新后的文档 |
| 版本发布 | 按语义化版本发布 | Release Notes |
| 知识沉淀 | 记录经验教训和最佳实践 | 知识库更新 |
...
| 要求项 | 说明 | 输出物 |
|--------|------|--------|
| 状态传递 | 传递当前状态和待办事项 | 状态报告 |
| 知识共享 | 团队分享会、技术文档 | 分享记录 |
| 经验总结 | 总结成功经验和改进点 | 复盘报告 |
| 下阶段准备 | 为下一阶段做好准备 | 准备清单 |
...
| 阶段 | 输入 | 输出 | 验证方式 | 负责人 |
|------|------|------|----------|--------|
| 分析建议 | 需求文档 | 技术方案 | 评审会议 | 架构师 |
| 实施推进 | 技术方案 | 源代码 | Code Review | 开发者 |
| 语法闭环 | 源代码 | 检查报告 | CI/CD | 自动化 |
| 测试同步 | 源代码 | 测试报告 | 测试执行 | 测试工程师 |
| 结果闭环 | 测试报告 | 发布版本 | 验收会议 | 产品经理 |
| 上下文衔接 | 发布版本 | 知识沉淀 | 复盘会议 | 团队 |

### 闭环异常处理

当某个阶段未通过验证时，必须：

1. **记录问题**：详细记录未通过的原因和现象
2. **回溯定位**：定位问题产生的阶段和原因
3. **修复验证**：修复问题并重新验证
4. **经验沉淀**：记录问题和解决方案，避免重复
...
**阶段名称**：[分析建议/实施推进/语法闭环/测试同步/结果闭环/上下文衔接]
**执行日期**：YYYY-MM-DD
**执行人**：[姓名]
